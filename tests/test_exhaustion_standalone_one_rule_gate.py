...
from scripts.run_exhaustion_standalone_one_rule_gate import (
    BASELINE_PROFILE,
    build_promotion_decision,
)


def _summary_row(*, cost: str, window: str, profile: str, geo: float, pf: float, max_dd_r: float, trades: int) -> dict:
    return {
        "cost_scenario": cost,
        "window": window,
...
def test_build_promotion_decision_promotes_candidate_when_all_gates_pass() -> None:
    ...
    summary_rows = [
        ...
        _summary_row(cost="base", window="full_2020", profile=candidate, geo=8.2, pf=1.20, max_dd_r=4.0, trades=14),
        _summary_row(cost="stress_x2", window="full_2020", profile=candidate, geo=1.8, pf=1.04, max_dd_r=4.4, trades=14),
    ]
    concentration_rows = [
...
def test_build_promotion_decision_rejects_when_profitability_gate_fails() -> None:
    ...
    summary_rows = [
        ...
        _summary_row(cost="base", window="full_2020", profile=candidate, geo=8.2, pf=1.05, max_dd_r=4.0, trades=14),
        _summary_row(cost="stress_x2", window="full_2020", profile=candidate, geo=1.8, pf=1.04, max_dd_r=4.4, trades=14),
    ]
    concentration_rows = [
        _concentration_row(cost="base", window="full_2020", profile=candidate, top3_share=55.0, best_year_share=70.0)
