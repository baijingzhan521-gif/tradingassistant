...
from scripts.run_switch_axis_band_management_one_rule_gate import (
    BASELINE_PRESET,
    PRESETS,
...
def _build_inputs(
    *,
    ...
    if btc_counts is None:
        btc_counts = {BASELINE_PRESET: 2, "long_active_k105": 2, "long_active_k110": 3, "long_active_k115": 1}
...
def test_build_promotion_decision_promotes_and_satisfies_btc_non_regression() -> None:
    winner = "long_active_k110"
    summary_rows, side_rows, concentration_rows, oos_rows, btc_counts = _build_inputs(
        candidate_overrides={
            winner: {"base_geo": 13.0, "base_cagr": 2.9, "oos_geo": 2.8},
            "long_active_k105": {"base_geo": 9.0, "base_cagr": 1.8},
            "long_active_k115": {"base_geo": 8.9, "base_cagr": 1.7},
        },
        btc_counts={BASELINE_PRESET: 2, "long_active_k105": 2, "long_active_k110": 3, "long_active_k115": 1},
    )
...
def test_build_promotion_decision_rejects_when_any_m1_gate_fails(override: dict, expected_fail_key: str) -> None:
    target = "long_active_k110"
    per_candidate = {
        preset: (override if preset == target else {"base_geo": 8.0, "base_cagr": 1.7})
        for preset in PRESETS
        if preset != BASELINE_PRESET
    }
...
def test_build_promotion_decision_keeps_status_promoted_even_if_btc_non_regression_fails() -> None:
    ...
    summary_rows, side_rows, concentration_rows, oos_rows, btc_counts = _build_inputs(
        candidate_overrides={
            target: {"base_geo": 13.0, "base_cagr": 2.9, "oos_geo": 2.8},
            "long_active_k105": {"base_geo": 9.0, "base_cagr": 1.8},
            "long_active_k115": {"base_geo": 8.9, "base_cagr": 1.7},
        },
        btc_counts={BASELINE_PRESET: 3, "long_active_k105": 2, "long_active_k110": 1, "long_active_k115": 1},
    )
    decision, _ = build_promotion_decision(
        summary_rows=summary_rows,
        side_rows=side_rows,
...
def test_build_btc_yearly_comparison_aggregates_counts() -> None:
    ...
    assert counts["long_active_k105"] == 1
