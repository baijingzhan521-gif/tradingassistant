...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "trend_confluence_standalone_one_rule_gate"
BASELINE_PROFILE = "swing_trend_confluence_setup_v1"
DEFAULT_PROFILES = [
    BASELINE_PROFILE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone one-rule promotion gate for trend-confluence candidates.")
        ...
        rows.append(
            {
                ...
                if promoted
                else "freeze_trend_confluence_and_keep_switch_baseline",
            }
        )
    return rows
...
def build_report(
    *,
    ...
    conclusion = (
        ...
        if promoted
        else "no candidate promoted; freeze trend-confluence family and move to next orthogonal one-rule gate."
    )
    return "\n".join(
        [
            "# Trend-Confluence Standalone One-Rule Gate",
            "",
            "- scope: standalone profitability promotion gate for trend-confluence candidates",
            "- baseline profile: `swing_trend_confluence_setup_v1`",
...
def main() -> None:
    ...
    print(f"saved trend-confluence standalone one-rule gate artifacts to {output_dir}")
