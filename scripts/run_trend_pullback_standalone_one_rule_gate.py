...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "trend_pullback_standalone_one_rule_gate"
BASELINE_PROFILE = "trend_pullback_v1"
DEFAULT_PROFILES = [
    BASELINE_PROFILE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone one-rule promotion gate for trend_pullback candidates.")
...
def build_trade_concentration(
    ...
    for (cost_scenario, window, strategy_profile), group in grouped:
        ...
        sub_yearly = yearly_frame[
            (yearly_frame["cost_scenario"] == cost_scenario)
            & (yearly_frame["window"] == window)
            & (yearly_frame["strategy_profile"] == strategy_profile)
        ]
        pos_year = sub_yearly.loc[sub_yearly["geometric_return_pct"] > 0.0, "geometric_return_pct"]
        if not pos_year.empty:
            best_year_geo = float(pos_year.max())
            best_year_share = float((best_year_geo / pos_year.sum()) * 100.0)
        else:
            best_year_geo = 0.0
        ...
        rows.append(
            {
                ...
                "next_route": (
                    ...
                    if promoted
                    else "freeze_trend_pullback_and_move_to_intraday_v2_round2"
                ),
            }
        )
    return rows
...
def build_report(
    *,
    ...
    conclusion = (
        ...
        if promoted
        else "no candidate promoted; freeze trend_pullback family and move to intraday_mtf_v2 one-rule gate."
    )
    return "\n".join(
        [
            "# Trend Pullback Standalone One-Rule Gate",
            "",
            "- scope: standalone promotion gate for trend_pullback family",
            "- baseline profile: `trend_pullback_v1`",
...
def main() -> None:
    ...
    print(f"saved trend-pullback standalone one-rule gate artifacts to {output_dir}")
