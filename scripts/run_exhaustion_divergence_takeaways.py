...
OUTPUT_DIR = ROOT / "artifacts/backtests/btc_exhaustion_divergence_v1"
MAINLINE = "swing_trend_long_regime_gate_v1"
EXHAUSTION_DIVERGENCE = "swing_exhaustion_divergence_v1_btc"
WINDOWS = {
    "two_year": (
...
def summarize_complementarity(
    ...
    exhaustion_trades: pd.DataFrame,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    mainline_monthly = (
        mainline_trades.groupby("signal_month")["pnl_r"].sum().rename("mainline_r").reset_index()
    )
    exhaustion_monthly = (
        exhaustion_trades.groupby("signal_month")["pnl_r"].sum().rename("exhaustion_r").reset_index()
    )
    monthly = mainline_monthly.merge(exhaustion_monthly, on="signal_month", how="outer").fillna(0.0)
    monthly["window"] = window
    monthly["combined_naive_r"] = monthly["mainline_r"] + monthly["exhaustion_r"]
    monthly["mainline_negative_exhaustion_positive"] = (
        (monthly["mainline_r"] < 0) & (monthly["exhaustion_r"] > 0)
    )
    monthly["opposite_sign"] = (monthly["mainline_r"] * monthly["exhaustion_r"]) < 0
    monthly = monthly.sort_values("signal_month").reset_index(drop=True)

    corr = None
    if len(monthly) >= 2 and monthly["mainline_r"].std() > 0 and monthly["exhaustion_r"].std() > 0:
        corr = float(monthly["mainline_r"].corr(monthly["exhaustion_r"]))

    offset_months = monthly[monthly["mainline_negative_exhaustion_positive"]].copy()
    summary = {
        "window": window,
        "months": int(len(monthly)),
        "monthly_corr": round(corr, 4) if corr is not None else None,
        "mainline_negative_exhaustion_positive_count": int(offset_months.shape[0]),
        "mainline_negative_exhaustion_positive_r_sum": round(
            float(offset_months["exhaustion_r"].sum()),
            4,
        ),
    ...
    monthly_rows = monthly[
        [
            ...
            "exhaustion_r",
            "combined_naive_r",
            "mainline_negative_exhaustion_positive",
            "opposite_sign",
        ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# BTC Exhaustion Divergence V1 Takeaways",
            "",
            "- 这轮不是给原趋势主线加一个 divergence filter，而是单独验证一个新的 exhaustion-reversal 家族。",
            "- `mainline` 仍按当前主线口径回测：`LONG 1R -> 3R scaled`，`SHORT 1.5R fixed`，`confirmed swing`。",
            "- `exhaustion_divergence` 使用策略自带的 `1R / 2R` 反抽目标；因此月度互补性只能看方向和分布，不能把 `combined_naive_r` 当真实组合收益。",
            "",
            "## Standalone Results",
            "",
            markdown_table(
            ...
            markdown_table(
                ...
                [
                    ...
                    ("mainline_negative_exhaustion_positive_count", "Main Neg / Exhaust Pos"),
                    ("mainline_negative_exhaustion_positive_r_sum", "Exhaust R In Those Months"),
            ...
            markdown_table(
                ...
                [
                    ...
                    ("exhaustion_r", "Exhaustion R"),
                    ("combined_naive_r", "Naive Combined R"),
                    ("mainline_negative_exhaustion_positive", "Main Neg / Exhaust Pos"),
...
def main() -> None:
    ...
    for window in WINDOWS:
        ...
        exhaustion_report = run_profile(window, EXHAUSTION_DIVERGENCE)

        (OUTPUT_DIR / f"{window}_{MAINLINE}_report.json").write_text(
            json.dumps(mainline_report.to_dict(), ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        (OUTPUT_DIR / f"{window}_{EXHAUSTION_DIVERGENCE}_report.json").write_text(
            json.dumps(exhaustion_report.to_dict(), ensure_ascii=False, indent=2),
            encoding="utf-8",
        )

        window_rows = report_to_rows(window, mainline_report) + report_to_rows(window, exhaustion_report)
        all_summary_rows.extend(window_rows)

        mainline_trades = trades_frame(mainline_report)
        exhaustion_trades = trades_frame(exhaustion_report)
        mainline_trades.to_csv(OUTPUT_DIR / f"{window}_{MAINLINE}_trades.csv", index=False)
        exhaustion_trades.to_csv(OUTPUT_DIR / f"{window}_{EXHAUSTION_DIVERGENCE}_trades.csv", index=False)
        ...
        top_offset_rows.extend(
            [
                ...
                if row["mainline_negative_exhaustion_positive"] or row["opposite_sign"]
            ]
        )

    top_offset_rows = sorted(
        top_offset_rows,
        key=lambda row: (row["window"], row["month"]),
    )
