...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "exhaustion_standalone_one_rule_gate"
BASELINE_PROFILE = "swing_exhaustion_divergence_v1_btc"
DEFAULT_PROFILES = [
    BASELINE_PROFILE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone one-rule promotion gate for exhaustion candidates.")
...
def geometric_return_pct(trades: list[BacktestTrade]) -> float:
    ...
    compounded = 1.0
    for trade in trades:
        compounded *= 1.0 + (float(trade.pnl_pct) / 100.0)
    return round((compounded - 1.0) * 100.0, 4)
        ...
        rows.append(
            {
                ...
                if promoted
                else "freeze_exhaustion_and_move_to_breakout",
            }
        )
    return rows
...
def build_report(
    *,
    ...
    conclusion = (
        ...
        if promoted
        else "no candidate promoted; freeze exhaustion family and move to breakout standalone gate."
    )
    return "\n".join(
        [
            "# Exhaustion Standalone One-Rule Gate",
            "",
            "- scope: standalone profitability promotion gate for exhaustion candidates",
            "- baseline profile: `swing_exhaustion_divergence_v1_btc`",
            "- this gate does not use offset/complementarity as a hard criterion",
            "",
...
def main() -> None:
    ...
    print(f"saved exhaustion standalone one-rule gate artifacts to {output_dir}")
