...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "neutral_range_standalone_one_rule_gate"
BASELINE_PROFILE = "swing_neutral_range_reversion_v1_btc"
DEFAULT_PROFILES = [
    BASELINE_PROFILE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone one-rule promotion gate for neutral-range candidates.")
        ...
        rows.append(
            {
                ...
                if promoted
                else "freeze_neutral_range_and_move_to_next_alpha_family",
            }
        )
    return rows
...
def build_report(
    *,
    ...
    conclusion = (
        ...
        if promoted
        else "no candidate promoted; freeze neutral-range family and move to next alpha family."
    )
    return "\n".join(
        [
            "# Neutral-Range Standalone One-Rule Gate",
            "",
            "- scope: standalone profitability promotion gate for neutral-range candidates",
            "- baseline profile: `swing_neutral_range_reversion_v1_btc`",
...
def main() -> None:
    ...
    print(f"saved neutral-range standalone one-rule gate artifacts to {output_dir}")
