...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "range_failure_standalone_one_rule_gate"
BASELINE_PROFILE = "swing_range_failure_v1_btc"
DEFAULT_PROFILES = [
    BASELINE_PROFILE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone one-rule promotion gate for range-failure candidates.")
...
def build_trade_concentration(trade_frame: pd.DataFrame, yearly_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if trade_frame.empty:
        return []
        ...
        rows.append(
            {
                ...
                "stress_full_2020_candidate_geometric_return_pct": round(float(stress_full["geometric_return_pct"]), 4),
                ...
                "status": status,
                "next_route": "parallel_compare_with_switch_baseline"
                if promoted
                else "freeze_range_failure_and_move_to_next_orthogonal_family",
            }
        )
    return rows
...
def build_report(
    *,
    ...
    conclusion = (
        ...
        if promoted
        else "no candidate promoted; freeze range-failure one-rule branch and move to next orthogonal family."
    )
    return "\n".join(
        [
            "# Range-Failure Standalone One-Rule Gate",
            "",
            "- scope: standalone profitability promotion gate for range-failure candidates",
            "- baseline profile: `swing_range_failure_v1_btc`",
            "- this stage is standalone gate only; no PnL summation with mainline baseline",
            "",
...
def main() -> None:
    ...
    print(f"saved range-failure standalone one-rule gate artifacts to {output_dir}")
