...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "switch_axis_band_management_one_rule_gate"
...
PRESETS: dict[str, dict[str, Any]] = {
    "flat_control": {"apply_active_long": False, "active_long_k": 1.0},
    "long_active_k105": {"apply_active_long": True, "active_long_k": 1.05},
    "long_active_k110": {"apply_active_long": True, "active_long_k": 1.10},
    "long_active_k115": {"apply_active_long": True, "active_long_k": 1.15},
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Axis/Band management one-rule gate on fixed switch baseline.")
...
def build_weighted_trades(trade_frame: pd.DataFrame, *, preset: str) -> pd.DataFrame:
    ...
    if bool(cfg["apply_active_long"]):
        long_active = (result["side"] == "LONG") & (result["risk_group"] == "active")
        raw_size = raw_size * long_active.map({True: float(cfg["active_long_k"]), False: 1.0}).astype(float)
    ...
    *,
        ...
        candidate_rows.append(
            {
                ...
                "all_pass": all_pass,
                "satisfied_result": all_pass and pass_btc_non_regression,
            }
        )
    ...
    decision = {
        ...
        "next_route": (
            ...
            else "freeze_axis_band_management_one_rule_after_single_round"
        ),
    }
...
def build_report(
    *,
    ...
    conclusion = (
        "候选通过 M1，且 BTC 年度跑赢年数未退步，达到本轮满意结果。"
        if decision_row["status"] == "promoted_management_overlay_candidate" and bool(decision_row["satisfied_result"])
        else "候选未达到本轮满意结果（可能是 M1 未过，或 BTC 年度诊断不达标）。"
    )

    return "\n".join(
        [
            "# Switch Axis/Band Management One-Rule Gate",
            "",
            "- baseline trades source: fixed switch (`simple_candidate_v2 -> challenger_managed`) with fixed cutover date",
            "- one-rule semantics: only LONG-active receives extra multiplier, then normalize full-window avg_size to 1.0",
...
def main() -> None:
    ...
    print(f"saved switch axis-band management one-rule gate artifacts to {output_dir}")
