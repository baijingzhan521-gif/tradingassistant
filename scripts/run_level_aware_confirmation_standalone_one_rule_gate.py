...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "level_aware_confirmation_standalone_one_rule_gate"
BASELINE_PROFILE = "swing_trend_level_aware_confirmation_v1"
DEFAULT_PROFILES = [
    BASELINE_PROFILE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Standalone one-rule promotion gate for level-aware confirmation candidates."
    )
        ...
        rows.append(
            {
                ...
                if promoted
                else "freeze_level_aware_confirmation_and_keep_switch_baseline",
            }
        )
    return rows
...
def build_report(
    *,
    ...
    conclusion = (
        ...
        if promoted
        else "no candidate promoted; freeze level-aware-confirmation family and move to next orthogonal one-rule gate."
    )
    return "\n".join(
        [
            "# Level-Aware Confirmation Standalone One-Rule Gate",
            "",
            "- scope: standalone profitability promotion gate for level-aware-confirmation candidates",
            "- baseline profile: `swing_trend_level_aware_confirmation_v1`",
...
def main() -> None:
    ...
    print(f"saved level-aware-confirmation standalone one-rule gate artifacts to {output_dir}")
