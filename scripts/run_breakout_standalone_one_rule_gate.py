...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "breakout_standalone_one_rule_gate"
BASELINE_PROFILE = "swing_breakout_v1_btc"
DEFAULT_PROFILES = [
    BASELINE_PROFILE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone one-rule promotion gate for breakout candidates.")
        ...
        rows.append(
            {
                ...
                if promoted
                else "freeze_breakout_and_move_to_neutral_range_gate",
            }
        )
    return rows
...
def build_report(
    *,
    ...
    conclusion = (
        ...
        if promoted
        else "no candidate promoted; freeze breakout family and move to neutral-range standalone gate."
    )
    return "\n".join(
        [
            "# Breakout Standalone One-Rule Gate",
            "",
            "- scope: standalone profitability promotion gate for breakout candidates",
            "- baseline profile: `swing_breakout_v1_btc`",
...
def main() -> None:
    ...
    print(f"saved breakout standalone one-rule gate artifacts to {output_dir}")
