...
from scripts.run_post_tp1_path_condition_matrix import (
    PROFILE_SPECS as PATH_PROFILE_SPECS,
    PathProfileSpec,
    precompute_path_features,
    run_profile as run_path_profile,
)
...
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "backtests" / "switch_post_tp1_path_one_rule_gate"
...
PRESETS: dict[str, dict[str, Any]] = {
    "flat_control": {"path_profile_name": None},
    "path_hold_2bars_4h_bullish": {"path_profile_name": "path_hold_if_2bars_above_ema21_and_4h_bullish"},
    "path_hold_3bars_4h_bullish": {"path_profile_name": "path_hold_if_3bars_above_ema21_and_4h_bullish"},
    "path_hold_3bars_4h_bullish_then_be_on_4h_loss": {
        "path_profile_name": "path_hold_if_3bars_above_ema21_and_4h_bullish_then_be_on_4h_loss"
    },
}
PATH_SPEC_BY_NAME: dict[str, PathProfileSpec] = {spec.name: spec for spec in PATH_PROFILE_SPECS}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Post-TP1 path-hold management one-rule gate on fixed switch baseline.")
...
def _path_replay_backtest_service(*, assumption_overrides: dict[str, Any] | None = None) -> BacktestService:
    assumptions = {
...
def prepare_path_replay_inputs(
    *,
    ...
    service = _path_replay_backtest_service()
    profile_inputs: dict[str, dict[str, Any]] = {}
    for strategy_profile in (simple_profile, challenger_strategy_profile):
        ...
        enriched = service._prepare_enriched_frames(strategy_profile, history)
        path_features = precompute_path_features(trigger_frame=enriched["1h"], four_h_frame=enriched["4h"])
        profile_inputs[strategy_profile] = {
            "strategy": strategy,
            "enriched": enriched,
...
def run_path_switch_case(
    *,
    path_spec: PathProfileSpec,
    ...
    for cost_scenario, overrides in cost_scenarios.items():
        for window, (window_start, window_end) in windows.items():
            for attempt in range(3):
                try:
                    service = _path_replay_backtest_service(assumption_overrides=overrides)
                    simple_ctx = shared_inputs[simple_profile]
                    challenger_ctx = shared_inputs[challenger_strategy_profile]
                    _, simple_trades, _ = run_path_profile(
                        service=service,
                        strategy=simple_ctx["strategy"],
                        symbol=SYMBOL,
                        strategy_profile=simple_profile,
                        spec=path_spec,
                        start=window_start,
                        end=window_end,
                        enriched=simple_ctx["enriched"],
                        path_features=simple_ctx["path_features"],
                    )
                    _, challenger_trades, _ = run_path_profile(
                        service=service,
                        strategy=challenger_ctx["strategy"],
                        symbol=SYMBOL,
                        strategy_profile=challenger_strategy_profile,
                        spec=path_spec,
                        start=window_start,
                        end=window_end,
                        enriched=challenger_ctx["enriched"],
                        path_features=challenger_ctx["path_features"],
                    )
                    all_rows.extend(
                        build_switch_rows(
    ...
    return build_trade_frame(all_rows)
    ...
    *,
    ...
    decision = {
        ...
        "next_route": (
            ...
            else "freeze_post_tp1_path_management_one_rule_after_single_round"
        ),
    }
...
def build_report(
    *,
    ...
    return "\n".join(
        [
            "# Switch Post-TP1 Path-Hold Management One-Rule Gate",
            "",
            "- baseline trades source: fixed switch (`simple_candidate_v2 -> challenger_managed`) with fixed cutover date",
            "- one-rule semantics: baseline sizing fixed at `long_1.15 / short_0.79 + LONG-active*1.15`; candidate delta only comes from post-TP1 path-hold rule",
...
def main() -> None:
    ...
    flat_trade_frame = case["trade_frame"].copy()
    flat_trade_frame = flat_trade_frame[flat_trade_frame["scenario_kind"] == SWITCH_SCENARIO_KIND].copy()
    if flat_trade_frame.empty:
        raise RuntimeError("No switch trades generated for the configured window.")
    for column in ("signal_time", "entry_time", "exit_time"):
        flat_trade_frame[column] = pd.to_datetime(flat_trade_frame[column], utc=True)
    flat_trade_frame["year"] = flat_trade_frame["signal_time"].dt.year.astype(int)
    flat_trade_frame = flat_trade_frame.sort_values(["cost_scenario", "window", "entry_time"]).reset_index(drop=True)

    windows = case["windows"]
    shared_path_inputs = prepare_path_replay_inputs(
        primary_start=primary_start,
        resolved_end=case["resolved_end"],
        simple_profile=SIMPLE_PROFILE,
        challenger_strategy_profile=CHALLENGER_STRATEGY_PROFILE,
    )

    preset_frames: dict[str, pd.DataFrame] = {BASELINE_PRESET: flat_trade_frame.copy()}
    for preset, cfg in PRESETS.items():
        if preset == BASELINE_PRESET:
            continue
        path_profile_name = str(cfg["path_profile_name"])
        path_spec = PATH_SPEC_BY_NAME.get(path_profile_name)
        if path_spec is None:
            raise KeyError(f"Unknown post-TP1 path profile: {path_profile_name}")
        preset_frames[preset] = run_path_switch_case(
            path_spec=path_spec,
            windows=windows,
            cost_scenarios=COST_SCENARIOS,
            simple_profile=SIMPLE_PROFILE,
            challenger_strategy_profile=CHALLENGER_STRATEGY_PROFILE,
            switch_date=switch_date,
            shared_inputs=shared_path_inputs,
        )
    ...
    for preset, frame in preset_frames.items():
        ...
        if not current.empty:
            ...
            current["year"] = current["signal_time"].dt.year.astype(int)
            current = current.sort_values(["cost_scenario", "window", "entry_time"]).reset_index(drop=True)
        current["preset"] = preset
        candidate_frames.append(current)

    candidate_trade_frame = pd.concat(candidate_frames, ignore_index=True) if candidate_frames else pd.DataFrame()
    labeled_trade_frame, label_coverage = attach_axis_band_risk_group(candidate_trade_frame)
    labeled_trade_frame["year"] = labeled_trade_frame["signal_time"].dt.year.astype(int)

    weighted_frames: list[pd.DataFrame] = []
    for preset in PRESETS:
        preset_labeled = labeled_trade_frame[labeled_trade_frame["preset"] == preset].copy()
        weighted_frames.append(build_weighted_trades(preset_labeled, preset=preset))
        ...
        json.dumps(
            {
                ...
                "cost_scenarios": COST_SCENARIOS,
                "switch_scenario_kind": SWITCH_SCENARIO_KIND,
                ...
                "challenger_strategy_profile": CHALLENGER_STRATEGY_PROFILE,
                "challenger_overlay_profile": CHALLENGER_OVERLAY_PROFILE,
                "path_profile_mapping": {preset: cfg["path_profile_name"] for preset, cfg in PRESETS.items()},
    ...
    print(f"saved switch post-tp1 path management one-rule gate artifacts to {output_dir}")
